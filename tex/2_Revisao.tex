...
\figura{figuras/resistencias}{Modelo de resistências segmentares do corpo humano utilizado na CF577BLE, com um resistor por segmento (braços $Z_{LA}$ e $Z_{RA}$, tronco $Z_{TR}$ e pernas $Z_{LL}$ e $Z_{RL}$) e os pontos de contato dos oito eletrodos nas mãos e nos pés. O tronco apresenta a menor resistência, pelo maior volume de fluidos e tecidos condutores, enquanto os membros têm resistência relativa maior}{fig:resistencias}{width=.5\linewidth}
...
O uso de modelos de linguagem (\emph{Large Language Models}, ou LLMs, como GPT e Gemini)
...
A revisão bibliográfica mostra que a bioimpedância elétrica é muito usada para estimar a composição
