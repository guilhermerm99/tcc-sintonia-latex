...
Alguns detalhes de implementação dos modelos estatísticos próprios do sistema (a correção
\emph{within-user} das medições e a calibração para o padrão DXA) são omitidos neste texto por
estarem em processo de proteção intelectual. São apresentados a abordagem adotada e os
resultados obtidos, suficientes para compreender e avaliar a solução, sem detalhar as fórmulas
e os algoritmos que constituem a contribuição original do trabalho.
...
próprio e uma inteligência artificial executada localmente, gerando análises
...
para o módulo de IA, que gera interpretações e recomendações
...
foi executada em servidor da Universidade de Brasília com aceleração por GPU. A
Figura~\ref{fig:arquitetura} apresenta a arquitetura geral e o fluxo de processamento do
\sintonia, do dispositivo de aquisição à interface, com o fluxo local (principal) e o de
contingência em nuvem.

\figura{figuras/arquitetura}{Arquitetura completa do sistema \sintonia, do dispositivo de aquisição à interface, com o fluxo local (principal) e o de contingência em nuvem}{fig:arquitetura}{width=\linewidth}
...
\section{Implementação do Assistente Virtual com SLM Local e Engenharia de Prompt}
...
informação, mas interpretá-la corretamente exige conhecimento do domínio e a capacidade de
relacionar os números às características de cada usuário. Para isso, o trabalho usa um SLM de
propósito geral (Qwen 2.5), orientado por um \emph{system prompt} com instruções e
\emph{guardrails}, sem treinamento específico do modelo. O comportamento no domínio vem
dessas instruções e dos dados já calculados que são injetados no \emph{prompt}, unindo
orientação por \emph{prompt}, privacidade e eficiência computacional. Nessa abordagem, o
modelo de linguagem não faz os cálculos biométricos: essas etapas ficam nos módulos de
processamento, e o SLM atua depois, produzindo interpretações contextualizadas.
...
genéricas para métricas específicas do domínio. Estudos recentes mostram que modelos menores,
bem direcionados a um domínio por instruções, podem ter desempenho igual ou melhor que os
generalistas em
...
\subsection{Regras de Interpretação e Instruções do System Prompt}

O comportamento no domínio não vem de treinamento do modelo, mas de um \emph{system prompt}
que reúne instruções, \emph{guardrails} e regras de interpretação fundamentadas na
literatura. Essas regras foram elaboradas a partir de artigos revisados por pares, protocolos
clínicos, diretrizes de avaliação nutricional e literatura sobre bioimpedância, além de
materiais técnicos da CF577BLE e da documentação do seu protocolo Bluetooth, o que permite ao
sistema interpretar corretamente os dados brutos. As referências adotadas orientam as
interpretações e ajudam a manter as análises alinhadas às evidências, reduzindo o risco de
alucinações e aumentando a confiabilidade.
...
A adaptação da assistente ao domínio não foi feita por treinamento adicional do modelo. Em vez
...
Na prática, a engenharia de \emph{prompt} do \sintonia\ combina três elementos. O primeiro é
um \emph{system prompt} extenso que define a persona ``Vida'', delimita o escopo (apenas
composição corporal e bioimpedância), impõe regras éticas (não emitir diagnóstico nem
recomendar medicamentos e orientar a busca por profissionais em casos extremos) e fixa o
estilo das respostas (português do Brasil, conciso, sem emojis). O segundo são os indicadores
biométricos já calculados pelo sistema, a anamnese e o histórico, injetados como contexto da
conversa. O terceiro são as regras de interpretação fundamentadas na literatura, que orientam
o que destacar em cada faixa de resultado. Complementam essa camada garantias implementadas
em código, independentes do modelo, como a trava de idioma, que regenera a resposta em
português caso o modelo desvie, e a verificação de procedência, que impede a assistente de
atribuir ao usuário informações que ele não forneceu.

A opção pela engenharia de \emph{prompt} em vez do \emph{fine-tuning} decorre do estágio do
projeto: o ajuste supervisionado exigiria um conjunto de exemplos de interpretação revisados
por especialistas, ainda não disponível, além de infraestrutura de treinamento e de validação
para evitar que o modelo aprendesse erros de domínio. A engenharia de \emph{prompt}, por sua
vez, tem custo de treinamento nulo, é transparente e pode ser atualizada apenas editando as
regras. Como evolução futura (Capítulo~\ref{cap:trabalhosfuturos}), o \emph{fine-tuning} com
técnicas eficientes como o QLoRA, apoiado em bases sintéticas validadas, poderia aumentar a
consistência das respostas e reduzir a latência, já que parte do conhecimento do domínio
passaria a residir no próprio modelo, permitindo \emph{prompts} mais curtos e, eventualmente,
um modelo base menor.

\subsection{Arquitetura de Inferência}

O modelo foi incorporado ao \sintonia\ como componente responsável pela
...
Em termos de abordagem, a correção é uma regressão linear \emph{within-user} por canal de
impedância (impedância total em 50~kHz, 20~kHz e 100~kHz e canais segmentares). Para cada
usuário, subtrai-se a sua linha de base (a média individual), de modo que só as variações
dentro do próprio usuário sejam modeladas e as diferenças fixas entre pessoas sejam
eliminadas. Sobre esses desvios, o modelo estima o quanto cada confundidor (não-jejum, água,
exercício e período menstrual) desloca o sinal e desconta esse deslocamento do valor medido.
Os confundidores não são binários: cada um entra com uma severidade entre 0 e 1, proporcional
à dose e ao tempo decorrido.

Para evitar estimativas instáveis quando o usuário tem poucas medições, o modelo usa uma
formulação de efeitos mistos com \emph{partial pooling} (Bayes empírico): a sensibilidade de
cada pessoa a cada fator é ``encolhida'' em direção à média da população e, quanto menos dados
o usuário tem, mais o modelo recorre ao comportamento global. O modelo também quantifica a
incerteza da correção, apresentada ao usuário como intervalo de confiança de 95\% no gráfico
de evolução. A correção só entra em ação depois de um número mínimo de medições com anamnese
preenchida (algumas em condições ideais e outras em condições alteradas), para que só atue
quando há dados suficientes para aprender o viés individual. Toda a implementação é feita em
Python puro, sem bibliotecas numéricas externas.

Os detalhes matemáticos da formulação (estimadores, coeficientes e a expressão da incerteza),
assim como os da calibração descrita na Seção~\ref{sec:testes}, foram omitidos por estarem em
processo de proteção intelectual. Ainda assim, a abordagem aqui descrita e os resultados da
Seção~\ref{sec:testes} permitem avaliar o funcionamento e a validade do método. Esse
tratamento do contexto de cada medição é um dos diferenciais da plataforma, porque leva em
conta fatores que os aplicativos comuns de bioimpedância costumam ignorar.
...
funciona como uma camada de comunicação que contextualiza os indicadores,
...
\figura{figuras/tela-principal}{Interface principal (dashboard) do aplicativo \sintonia, com os indicadores corporais, a leitura interpretativa e o gráfico de evolução}{fig:tela}{width=\linewidth}

\figura{figuras/whatsapp}{Interação do usuário com a assistente virtual Vida do \sintonia\ por meio do WhatsApp}{fig:whatsapp}{width=.62\linewidth}
...
já implementa um modelo de calibração por pessoa, que ajusta a estimativa da FFM ao padrão
DXA, mas sua ativação depende de pares de medições CF577BLE e DXA da mesma pessoa, ainda não
coletados.
...
\begin{table}[H]
    \centering
    \caption{Resultado do estudo de simulação da calibração CF577BLE $\rightarrow$ DXA (validação \emph{leave-one-out}).}
...
DXA), o mesmo formato que receberá os dados reais quando forem coletados. No estudo de
simulação (30 pares), o modelo aprendeu um fator de calibração individual e a FFM estimada
aproximou-se da referência: o erro absoluto médio em relação ao DXA caiu de cerca de 1,3~kg
(sem calibração) para 0,3~kg (com calibração). Assim, quando houver dados de DXA reais, basta
substituir o CSV para que a calibração por pessoa passe a valer, sem qualquer outra alteração
no sistema.
...
\begin{table}[H]
    \centering
    \caption{Estudo de simulação da correção \emph{within-user}: viés injetado por confundidor e viés recuperado pelo modelo (impedância de 50~kHz).}
    \label{tab:correcao}
    \begin{tabular}{lcc}
        \toprule
